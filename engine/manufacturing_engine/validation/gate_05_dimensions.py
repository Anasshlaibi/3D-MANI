...
def run_gate_05(mass_props: MassProperties, program: ProgramNode, revision: int = 1) -> Dict[str, Any]:
    ...
    result = {
        ...
        "diagnosticMessage": f"Volume ({mass_props.volume_ml} ml) meets requirement (>= {target_vol} ml). Height ({mass_props.bounding_box_max[1]} mm) within envelope." if passed else f"Dimension constraint violation: Target volume {target_vol} ml vs measured {mass_props.volume_ml} ml.",
        "evidence": {
