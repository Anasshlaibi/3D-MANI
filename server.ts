...
app.get('/api/health', async (req, res) => {
  ...
  try {
    ...
    if (pyRes.ok) {
      ...
      pythonEngineStatus = data.status === 'ok' ? 'online (build123d/OCCT)' : 'degraded';
    }
  } catch (e) {}
...
Return ONLY valid JSON matching this schema:
{
  ...
  "generatedDSL": string (valid Manufacturing DSL syntax)
      ...
      console.warn('Gemini extraction error, falling back to deterministic extraction:', err?.message);
    }
  }

  // Deterministic fallback if API key not available or transient error
  const lower = prompt.toLowerCase();
  const isCup = lower.includes('cup') || lower.includes('drink') || lower.includes('mug');
  const isEnclosure = lower.includes('enclosure') || lower.includes('box') || lower.includes('case');
  const isFitting = lower.includes('fitting') || lower.includes('pipe') || lower.includes('adapter');
  ...
  return res.json({
    ...
    data: {
      ...
      material,
      explicitLocked: [
        { name: 'capacity', value: capacity, unit: 'ml', reason: 'Explicitly extracted from user prompt' },
        { name: 'stackable', value: lower.includes('stack'), unit: 'boolean', reason: 'Stated in prompt' },
      ],
      inferred: [
        { name: 'process', value: 'INJECTION_MOLDING', unit: 'enum', reason: 'High-volume thin-wall thermoplastic geometry' },
        { name: 'material', value: material, unit: 'enum', reason: 'Optimal for thin-wall food/consumer or enclosure durability' },
        { name: 'wall_thickness', value: material === 'PP' ? 1.8 : 2.2, unit: 'mm', reason: 'Conforms to process flow rules' },
        { name: 'draft_angle', value: 1.75, unit: 'deg', reason: 'Standard draft for clean mold ejection' },
      ],
      missingCritical: [
        { name: 'production_quantity', recommendedDefault: 100000, unit: 'parts', reason: 'Needed to determine tooling steel grade (P20 vs H13)' },
        { name: 'food_contact_requirement', recommendedDefault: true, unit: 'boolean', reason: 'Determines FDA virgin resin certification' },
        { name: 'max_height', recommendedDefault: 120, unit: 'mm', reason: 'Constrains shelf or packaging envelope' },
      ],
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`AI Manufacturing Design Engine server running on http://0.0.0.0:${PORT}`);
  });
}
